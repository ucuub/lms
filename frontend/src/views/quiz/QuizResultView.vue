        ...
        <div class="space-y-5">
          <div v-for="(a, idx) in result.answers" :key="a.questionId"
            ...
            <div v-if="a.type !== 'Essay'" class="ml-7 space-y-1 text-sm">
              ...
              <!-- Kunci jawaban hanya tampil jika instruktur mengizinkan -->
              <p v-if="a.correctAnswer && !a.isCorrect" class="text-green-700">
                Jawaban benar: <strong>{{ a.correctAnswer }}</strong>
              </p>
            </div>

            <div v-else class="ml-7 text-sm">
              <p v-if="a.essayAnswer" class="text-gray-600">Jawaban essay: {{ a.essayAnswer }}</p>
